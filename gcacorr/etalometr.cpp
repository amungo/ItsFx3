...
int Etalometr::LoadEtalonsFromFiles()
{
    ...
    for ( int th_idx = startThettaIdx; th_idx < 180 - startThettaIdx; th_idx += etalonStep ) {
        for ( int ph_idx = startPhiIdx; ph_idx < 180 - startPhiIdx; ph_idx += etalonStep ) {
            PhasesDiff_t x;
            ...
            etalons[th_idx - startThettaIdx][ph_idx - startPhiIdx] = x;
        }
    }
