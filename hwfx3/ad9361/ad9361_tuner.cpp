...
ad9361_device_t::ad9361_device_t(DeviceControlIOIfce *device_cmd_io, ad9361_params *params) :
    _io_iface( device_cmd_io ),
    _client_params( params )
{
    using namespace std::placeholders;
    _rx_filters["LPF_TIA"] = filter_query_helper( std::bind(&ad9361_device_t::_get_filter_lp_tia_sec, this, _1),
                                                  std::bind(&ad9361_device_t::_set_filter_lp_tia_sec, this, _1, _3) );

    _rx_filters["LPF_BB" ] = filter_query_helper( std::bind(&ad9361_device_t::_get_filter_lp_bb, this, _1),
                                                  std::bind(&ad9361_device_t::_set_filter_lp_bb, this, _1, _3) );

    _rx_filters["HB_3"   ] = filter_query_helper( std::bind(&ad9361_device_t::_get_filter_hb_3,  this, _1),
                                                  std::bind(&ad9361_device_t::_set_filter_dummy, this, _1, _2, _3) );

    _rx_filters["DEC_3"  ] = filter_query_helper( std::bind(&ad9361_device_t::_get_filter_dec_int_3,  this, _1),
                                                  std::bind(&ad9361_device_t::_set_filter_dummy,      this, _1, _2, _3) );

    _rx_filters["HB_2"   ] = filter_query_helper( std::bind(&ad9361_device_t::_get_filter_hb_2,  this, _1),
                                                  std::bind(&ad9361_device_t::_set_filter_dummy, this, _1, _2, _3) );

    _rx_filters["HB_1"   ] = filter_query_helper( std::bind(&ad9361_device_t::_get_filter_hb_1,  this, _1),
                                                  std::bind(&ad9361_device_t::_set_filter_dummy, this, _1, _2, _3) );

    _rx_filters["FIR_1"  ] = filter_query_helper( std::bind(&ad9361_device_t::_get_filter_fir, this, _1, _2),
                                                  std::bind(&ad9361_device_t::_set_filter_fir, this, _1, _2, _3));

    _tx_filters["LPF_SECONDARY"]=filter_query_helper(std::bind(&ad9361_device_t::_get_filter_lp_tia_sec, this, _1),
                                                     std::bind(&ad9361_device_t::_set_filter_lp_tia_sec, this, _1, _3));

    _tx_filters["LPF_BB"]=filter_query_helper(std::bind(&ad9361_device_t::_get_filter_lp_bb, this, _1),
                                              std::bind(&ad9361_device_t::_set_filter_lp_bb, this, _1, _3));

    _tx_filters["HB_3"  ]=filter_query_helper(std::bind(&ad9361_device_t::_get_filter_hb_3,  this, _1),
                                              std::bind(&ad9361_device_t::_set_filter_dummy, this, _1, _2, _3) );

    _tx_filters["INT_3" ]=filter_query_helper(std::bind(&ad9361_device_t::_get_filter_dec_int_3, this, _1),
                                              std::bind(&ad9361_device_t::_set_filter_dummy,     this, _1, _2, _3) );

    _tx_filters["HB_2"  ]=filter_query_helper(std::bind(&ad9361_device_t::_get_filter_hb_2,  this, _1),
                                              std::bind(&ad9361_device_t::_set_filter_dummy, this, _1, _2, _3) );

    _tx_filters["HB_1"  ]=filter_query_helper(std::bind(&ad9361_device_t::_get_filter_hb_1,  this, _1),
                                              std::bind(&ad9361_device_t::_set_filter_dummy, this, _1, _2, _3) );

    _tx_filters["FIR_1" ]=filter_query_helper(std::bind(&ad9361_device_t::_get_filter_fir,   this, _1, _2),
                                              std::bind(&ad9361_device_t::_set_filter_fir,   this, _1, _2, _3));
...
void ad9361_device_t::_setup_rx_fir(size_t num_taps, int32_t decimation)
{
    if (! (decimation == 1 || decimation == 2 || decimation == 4)) {
        throw std::runtime_error("[ad9361_device_t] Invalid Rx FIR decimation.");
    }
    ...
    for (size_t i = 0; i < num_taps; i++) {
        ...
        default:
            delete [] coeffs;
            throw std::runtime_error("[ad9361_device_t] Unsupported number of Rx FIR taps.");
        }
    }

    _program_fir_filter(RX, CHAIN_BOTH, num_taps, coeffs);
    delete [] coeffs;
}
...
{
    if (! (interpolation == 1 || interpolation == 2 || interpolation == 4)) {
        throw std::runtime_error("[ad9361_device_t] Invalid Tx FIR interpolation.");
    }
    if (interpolation == 1 && num_taps > 64) {
        throw std::runtime_error("[ad9361_device_t] Too many Tx FIR taps for interpolation value.");
    }
    uint16_t* coeffs = new uint16_t[num_taps];
    for (size_t i = 0; i < num_taps; i++) {
        switch (num_taps) {
        ...
        default:
            delete [] coeffs;
            throw std::runtime_error("[ad9361_device_t] Unsupported number of Tx FIR taps.");
        }
    }

    _program_fir_filter(TX, CHAIN_BOTH, num_taps, coeffs);
    delete [] coeffs;
}
...
void ad9361_device_t::_calibrate_lock_bbpll()
{
    ...
    while (!(_io_iface->peek8(0x05e) & 0x80)) {
        if (count > 1000) {
            throw std::runtime_error("[ad9361_device_t] BBPLL not locked");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_synth_charge_pumps()
{
    ...
    if ((_io_iface->peek8(0x017) & 0x0F) != 5) {
        throw std::runtime_error("[ad9361_device_t] AD9361 not in ALERT during cal");
    }
    ...
    while (!(_io_iface->peek8(0x244) & 0x80)) {
        if (count > 5) {
            throw std::runtime_error("[ad9361_device_t] RX charge pump cal failure");
            break;
        }
        count++;
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    ...
    while (!(_io_iface->peek8(0x284) & 0x80)) {
        if (count > 5) {
            throw std::runtime_error("[ad9361_device_t] TX charge pump cal failure");
            break;
        }
        count++;
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
...
double ad9361_device_t::_calibrate_baseband_rx_analog_filter(double req_rfbw)
{
    ...
    {
        std::cerr << "baseband bandwidth too large for current sample rate. Setting bandwidth to: "<<_baseband_bw;
    ...
    while (_io_iface->peek8(0x016) & 0x80) {
        if (count > 100) {
            throw std::runtime_error("[ad9361_device_t] RX baseband filter cal FAILURE");
            break;
        }
        count++;
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
...
double ad9361_device_t::_calibrate_baseband_tx_analog_filter(double req_rfbw)
{
    ...
    while (_io_iface->peek8(0x016) & 0x40) {
        if (count > 100) {
            throw std::runtime_error("[ad9361_device_t] TX baseband filter cal FAILURE");
            break;
        }
...
double ad9361_device_t::_calibrate_secondary_tx_filter(double req_rfbw)
{
    ...
    if ((bbbw_mhz * 2) <= 9) {
        ...
        throw std::runtime_error("[ad9361_device_t] Cal2ndTxFil: INVALID_CODE_PATH bad bbbw_mhz");
        reg0d0 = 0x00;
    }
...
double ad9361_device_t::_calibrate_rx_TIAs(double req_rfbw)
{
    ...
    if (ceil_bbbw_mhz <= 3) {
        ...
        throw std::runtime_error("[ad9361_device_t] CalRxTias: INVALID_CODE_PATH bad bbbw_mhz");
    }
...
void ad9361_device_t::_calibrate_baseband_dc_offset()
{
    ...
    while (_io_iface->peek8(0x016) & 0x01) {
        if (count > 100) {
            throw std::runtime_error("[ad9361_device_t] Baseband DC Offset Calibration Failure");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_rf_dc_offset()
{
    ...
    while (_io_iface->peek8(0x016) & 0x02) {
        if (count > 200) {
            throw std::runtime_error("[ad9361_device_t] RF DC Offset Calibration Failure");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_rx_quadrature()
{
    ...
    while (_io_iface->peek8(0x016) & 0x20) {
        if (count > 1000) {
            throw std::runtime_error("[ad9361_device_t] Rx Quadrature Calibration Failure");
            break;
        }
        count++;
        std::this_thread::sleep_for(std::chrono::milliseconds(5));
...
void ad9361_device_t::_tx_quadrature_cal_routine() {
    ...
    if (max_cal_freq > bbbw)
        throw std::runtime_error("[ad9361_device_t] max_cal_freq > bbbw");
    ...
    while (_io_iface->peek8(0x016) & 0x10) {
        if (count > 100) {
            throw std::runtime_error("[ad9361_device_t] TX Quadrature Calibration Failure");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_tx_quadrature()
{
    ...
    if ((_io_iface->peek8(0x017) & 0x0F) != 5) {
        throw std::runtime_error("[ad9361_device_t] TX Quad Cal started, but not in ALERT");
    }
...
void ad9361_device_t::_program_gain_table() {
    ...
    if (_rx_freq < 1300e6) {
        ...
        throw std::runtime_error("[ad9361_device_t] Wrong _rx_freq value");
        new_gain_table = 1;
    }
...
void ad9361_device_t::_setup_synth(direction_t direction, double vcorate)
{
    ...
    if (vcoindex > 53)
        throw std::runtime_error("[ad9361_device_t] vcoindex > 53");
    ...
    if (direction == RX) {
        ...
        throw std::runtime_error("[ad9361_device_t] [_setup_synth] INVALID_CODE_PATH");
    }
}
...
{
    fprintf( stderr, "[ad9361_device_t::_tune_bbvco] rate=%.10f\n", rate );
    ...
    if (i == 7)
        throw std::runtime_error("[ad9361_device_t] _tune_bbvco: wrong vcorate");

    fprintf( stderr, "[ad9361_device_t::_tune_bbvco] vcodiv=%d vcorate=%.10f\n", vcodiv, vcorate );
    /* Fo = Fref * (Nint + Nfrac / mod) */
    int nint = static_cast<int>(vcorate / fref);
    fprintf( stderr, "[ad9361_device_t::_tune_bbvco] (nint)=%.10f\n", vcorate / fref);
    int nfrac = static_cast<int>(round(((vcorate / fref) - (double) nint) * (double) modulus));
    fprintf( stderr, "[ad9361_device_t::_tune_bbvco] (nfrac)=%.10f\n", ( (vcorate / fref) - (double) nint) * (double) modulus );
    fprintf( stderr, "[ad9361_device_t::_tune_bbvco] nint=%d nfrac=%d\n", nint, nfrac );
...
double ad9361_device_t::_tune_helper(direction_t direction, const double value)
{
    ...
    if (i == 7)
        throw std::runtime_error("[ad9361_device_t] RFVCO can't find valid VCO rate!");
    ...
    if (direction == RX) {
        ...
        if (value < _client_params->get_band_edge(AD9361_RX_BAND0))
        {
            ...
            fprintf( stderr, "_tune_helper: Port C, balanced\n" );
                 ...
                 && ( value <  _client_params->get_band_edge(AD9361_RX_BAND1) ) )
        {
            ...
            fprintf( stderr, "_tune_helper: Port B, balanced\n" );

        } else if ( ( value >= _client_params->get_band_edge(AD9361_RX_BAND1) )
                 && ( value <= 6e9 ) )
        {
            _regs.inputsel = (_regs.inputsel & 0xC0) | 0x03; // Port A, balanced
            fprintf( stderr, "_tune_helper: Port A, balanced\n" );

        } else
        {
            throw std::runtime_error("[ad9361_device_t] [_tune_helper] INVALID_CODE_PATH");
        }
        ...
        if ((_io_iface->peek8(0x247) & 0x02) == 0) {
            throw std::runtime_error("[ad9361_device_t] RX PLL NOT LOCKED");
        }
        ...
        if ((_io_iface->peek8(0x287) & 0x02) == 0) {
            throw std::runtime_error("[ad9361_device_t] TX PLL NOT LOCKED");
        }
...
double ad9361_device_t::_setup_rates(const double rate)
{
    ...
    fprintf( stderr, "[ad9361_device_t::_setup_rates] rate=%.6d\n", rate );
    ...
    if (rate < 0.33e6) {
        ...
        throw std::runtime_error("[ad9361_device_t] [_setup_rates] INVALID_CODE_PATH");
    }

    fprintf( stderr, "[ad9361_device_t::_setup_rates] divfactor=%d\n", divfactor );
    ...
    fprintf( stderr, "[ad9361_device_t::_setup_rates] adcclk=%f\n", adcclk );
...
void ad9361_device_t::initialize()
{
    ...
    default:
        throw std::runtime_error("[ad9361_device_t] NOT IMPLEMENTED");
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(20));
...
double ad9361_device_t::set_clock_rate(const double req_rate)
{
    ...
    if (req_rate > 61.44e6) {
        throw std::runtime_error("[ad9361_device_t] Requested master clock rate outside range");
    }

    fprintf( stderr, "[ad9361_device_t::set_clock_rate] req_rate=%.10f\n", req_rate );
    ...
    default:
        throw std::runtime_error("[ad9361_device_t] [set_clock_rate:1] AD9361 in unknown state");
        break;
    };
    ...
    fprintf( stderr, "[ad9361_device_t::set_clock_rate] rate=%.10f\n", rate );
    ...
    default:
        throw std::runtime_error("[ad9361_device_t] [set_clock_rate:2] AD9361 in unknown state");
        break;
    };
...
double ad9361_device_t::tune(direction_t direction, const double value)
{
    ...
    if (direction == RX) {
        ...
        throw std::runtime_error("[ad9361_device_t] [tune] INVALID_CODE_PATH");
    }
...
double ad9361_device_t::_get_temperature(const double cal_offset, const double timeout)
{
    ...
    while(((_io_iface->peek8(0x00C) >> 1) & 0x01) == 0) {
        ...
        if( elapsed.count() > (timeout*1000.0) )
        {
            throw std::runtime_error("[ad9361_device_t] timeout while reading temperature");
        }
    }
...
void ad9361_device_t::set_dc_offset_auto(direction_t direction, const bool on)
{
    if (direction == RX) {
        ...
        throw std::runtime_error("[ad9361_device_t] [set_dc_offset_auto] Tx DC tracking not supported");
    }
}
...
void ad9361_device_t::set_iq_balance_auto(direction_t direction, const bool on)
{
    if (direction == RX) {
        ...
        throw std::runtime_error("[ad9361_device_t] [set_iq_balance_auto] Tx IQ tracking not supported");
    }
}
...
void ad9361_device_t::_setup_agc(chain_t chain, gain_mode_t gain_mode)
{
    ...
    {
        throw std::runtime_error("[ad9361_device_t] Wrong value for chain");
    }
    ...
    switch (gain_mode) {
        ...
        case GAIN_MODE_FAST_AGC:
            ...
            break;
        default:
            throw std::runtime_error("[ad9361_device_t] Gain mode does not exist");
    }
...
{
    if(direction == RX) {
        if (! _rx_filters[name].get)
        {
            throw std::runtime_error("ad9361_device_t::get_filter this filter can not be read.");
        }
        return _rx_filters[name].get(direction, chain);
    } else if (direction == TX) {
        if (! _tx_filters[name].get)
        {
            throw std::runtime_error("ad9361_device_t::get_filter this filter can not be read.");
        }
    ...
    throw std::runtime_error("ad9361_device_t::get_filter wrong direction parameter.");
}
...
void ad9361_device_t::set_filter(direction_t direction, chain_t chain, const std::string &name, filter_info_base::sptr filter)
{
    ...
    if(direction == RX) {
        if(! _rx_filters[name].set)
        {
            throw std::runtime_error("ad9361_device_t::set_filter this filter can not be written.");
        }
        _rx_filters[name].set(direction, chain, filter);
    } else if (direction == TX) {
        if(! _tx_filters[name].set)
        {
            throw std::runtime_error("ad9361_device_t::set_filter this filter can not be written.");
        }
...
void ad9361_device_t::_set_fir_taps(direction_t direction, chain_t chain, const std::vector<int16_t>& taps)
{
    ...
    {
        uint16_t* coeffs = new uint16_t[num_taps_avail];
        for (size_t i = 0; i < num_taps_avail; i++)
        {
            coeffs[i] = uint16_t(taps[i]);
        }
        _program_fir_filter(direction, chain, num_taps_avail, coeffs);
        delete [] coeffs;
    } else if(num_taps < num_taps_avail){
        throw std::runtime_error("ad9361_device_t::_set_fir_taps not enough coefficients.");
    } else {
        throw std::runtime_error("ad9361_device_t::_set_fir_taps too many coefficients.");
    }
}
...
std::vector<int16_t> ad9361_device_t::_get_fir_taps(direction_t direction, chain_t chain)
{
    ...
    if(chain == CHAIN_1)
    {
        ...
        throw std::runtime_error("[ad9361_device_t] Can not read both chains synchronously");
    }
...
void ad9361_device_t::_set_filter_dummy(ad9361_device_t::direction_t, ad9361_device_t::chain_t, filter_info_base::sptr) {
    // nop
}
